...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model: int, num_attention_heads: int):
        ...
        self.heads = nn.ModuleList(AttentionHead(d_model, self.d_head, self.d_head, self.d_head) for i in range(num_attention_heads))
    ...
    def forward(self, hidden_state):
        ...
        concat = torch.cat(head_outputs, dim=-1)  # (B, L, num_heads * d_head)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        hidden_state = self.feed_forward(x)
        x = x + hidden_state  # Residual connection
